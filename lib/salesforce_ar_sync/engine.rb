    ...
    initializer 'salesforce_ar_sync.load_app_instance_data' do |app|
      SalesforceArSync.setup do |config|
        ...
        if FileTest.exist?("#{Rails.root}/config/salesforce_ar_sync.yml")
          ...
          SalesforceArSync.config['SYNC_QUEUE'] = config['sync_queue']&.to_sym || :default
        end
        ...
        SalesforceArSync.config['SYNC_QUEUE'] = ENV['SALESFORCE_AR_SYNC_SYNC_QUEUE'] if ENV['SALESFORCE_AR_SYNC_SYNC_QUEUE']
